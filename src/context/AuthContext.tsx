...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [allAcademyUsers, setAllAcademyUsers] = useState<User[]>(() => {
    return getStoredItem<User[]>(LOCAL_STORAGE_ALL_USERS_KEY, ACADEMY_USERS);
  });

  const [currentUser, setCurrentUser] = useState<User>(() => {
    return getStoredItem<User>(LOCAL_STORAGE_USER_KEY, CURRENT_USER);
  });

  useEffect(() => {
    setStoredItem(LOCAL_STORAGE_USER_KEY, currentUser);
  }, [currentUser]);

  useEffect(() => {
    setStoredItem(LOCAL_STORAGE_ALL_USERS_KEY, allAcademyUsers);
  }, [allAcademyUsers]);

  const isDomainValid = (email: string): boolean => {
    const trimmed = email.trim().toLowerCase();
    return trimmed.endsWith('@mpesafoundationacademy.ac.ke');
  };
  ...
  const loginWithAcademyEmail = (
    ...
    if (!isDomainValid(cleanEmail)) {
      return {
        success: false,
        message: 'Access Denied: MFA-VEXPEX is strictly restricted to M-PESA Foundation Academy. Only @mpesafoundationacademy.ac.ke email addresses are authorized.'
      };
    }
    ...
    // Create new Academy verified user
  ...
  const logout = () => {
    // Reset to default Stephen Kipsang or prompt auth
  ...
  return (
    <AuthContext.Provider
      value={{
        currentUser,
        allAcademyUsers,
        loginWithAcademyEmail,
        switchUser,
        updateProfile,
        logout,
        isDomainValid,
        activeHouse: currentUser.house
      }}
    >
      {children}
    </AuthContext.Provider>
